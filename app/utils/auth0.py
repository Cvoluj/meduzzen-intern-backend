...
class VerifyToken():
    ...
    def __init__(self, token, permissions=None, scopes=None):
        ...
        # the keys available
        jwks_url = f'https://{server_setting.DOMAIN}/.well-known/jwks.json'
    ...
    def verify(self):
        ...
        try: 
            payload = jwt.decode(
                self.token,
                self.signing_key,
                algorithms=server_setting.AUTH0_ALGORITHM,
                audience=server_setting.API_AUDIENCE,
                issuer=server_setting.ISSUER,
            )
